using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace SpellWork.Enums
{
    /// <summary>
    ///
    /// </summary>
    public enum SpellFamilyNames
    {
        SPELL_FAMILY_GENERIC                 = 00,
        SPELL_FAMILY_UNK1                    = 01, // events, holidays
        // unused                            = 02,
        SPELL_FAMILY_MAGE                    = 03,
        SPELL_FAMILY_WARRIOR                 = 04,
        SPELL_FAMILY_WARLOCK                 = 05,
        SPELL_FAMILY_PRIEST                  = 06,
        SPELL_FAMILY_DRUID                   = 07,
        SPELL_FAMILY_ROGUE                   = 08,
        SPELL_FAMILY_HUNTER                  = 09,
        SPELL_FAMILY_PALADIN                 = 10,
        SPELL_FAMILY_SHAMAN                  = 11,
        SPELL_FAMILY_UNK2                    = 12, // 2 spells (silence resistance)
        SPELL_FAMILY_POTION                  = 13,
        // unused                            = 14,
        SPELL_FAMILY_DEATHKNIGHT             = 15,
        // unused                            = 16,
        SPELL_FAMILY_PET                     = 17,
        SPELL_FAMILY_MINIGAME                = 50,
    };
}
